...
public class SelectSceneUI : MonoBehaviour
{
    ...
    public void SelectLvl(int lvl) { DataManager.instance.SetLvl(lvl); }
    public void SelectMode(int mode) { DataManager.instance.SetMode(mode); }

    public void SelectP1Chara(int chara)
    {
        if (DataManager.instance.GetP1Chara() != 0)
            return;
        // turn off SSUI > SetChara(2) > SelCharaBG(0) > chara0BG(1) ~ chara3BG(4)
        for (int a = 1; a < 5; a++)
        {
            if (a == DataManager.instance.GetP1Chara())
                continue;
        ...
        if (DataManager.instance.GetP2Chara() == 0)
        {
            p1chara = chara;
        }
    }
    public void SelectP2Chara(int chara)
    {
        ...
        if (DataManager.instance.GetMode() != 2)
            return;
        if (DataManager.instance.GetMode() == 2 && DataManager.instance.GetP1Chara() != 0)
    ...
    {
        DataManager.instance.SetP1Chara(0);
        DataManager.instance.SetP2Chara(0);
    }
    public void ConfirmChara(int opt)
    {
        ...
        {
            case 1: // p1 chara
                DataManager.instance.SetP1Chara(p1chara);
                break;
            case 2: // p2 chara
                DataManager.instance.SetP2Chara(p2chara);
                break;
        }
        if (DataManager.instance.GetP1Chara() != 0 && (DataManager.instance.GetMode() == 2 ? DataManager.instance.GetP2Chara() != 0 : true))
        {
    ...
    public void ConfirmName()
    {
        p1Name = IFp1.text;
        p2Name = IFp2.text;
        DataManager.instance.SetP1Name(p1Name);
        DataManager.instance.SetP2Name(p2Name);
